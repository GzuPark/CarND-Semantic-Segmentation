...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    layer4_upsample = tf.layers.conv2d_transpose(layer7_conv_1x1, num_classes, 4, 2,
                                                 padding='same',
                                                 kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    layer3_upsample = tf.layers.conv2d_transpose(layer4_out, num_classes, 4, 2,
                                                 padding='same',
                                                 kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    output = tf.layers.conv2d_transpose(layer3_out, num_classes, 16, 8,
                                        padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    with open('log_loss', 'w', encoding='utf-8') as f:
        f.writelines(['{:.4f}\n'.format(log) for log in log_loss])
...
def run():
    ...
    with tf.Session() as sess:
        ...
        correct_label = tf.placeholder(tf.int32, [None, None, None, num_classes], name='crrect_label')
